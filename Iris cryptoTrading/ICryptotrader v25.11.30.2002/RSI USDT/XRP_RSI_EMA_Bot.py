...
def place_order(signal):
    try:
        ...
        if signal == 'BUY':
            ...
            print(f"✅ LONG abierto a ${entry_price:.4f}")
            ...
            print(f"🎯 TP: ${tp_price:.4f} (+{TAKE_PROFIT_PCT*100}%)")
            print(f"🛑 SL: ${sl_price:.4f} (-{STOP_LOSS_PCT*100}%)")
            ...
            entry_price = float(order.get('average', current_price))
            print(f"✅ SHORT abierto a ${entry_price:.4f}")
    ...
    while True:
        try:
            ...
            if signal:
                ...
                print(f"📊 EMA9: {df['ema_fast'].iloc[-1]:.4f} | EMA21: {df['ema_slow'].iloc[-1]:.4f}")
